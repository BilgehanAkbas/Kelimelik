-- Kelimelik v1.2.34 — Efe nerf
-- Efe daha kolay bir bot olur: daha sık maçı bitiremez, daha geç çözer
-- ve erken oyunda fazla güçlü bilgi toplamaması için yeşil sınırı alır.
...
begin
  ...
  -- Efe belirgin biçimde daha zayıf; Defne orta, Atlas güçlü.
  if (clean_bot='efe' and fail_roll<0.58)
  ...
  then
    ...
    target:=case clean_bot
      ...
      else 4+floor(random()*greatest(1,attempts-3))::integer
    end;
  ...
  insert into public.bot_match_secrets(match_id,answer_version,answer_word) values(match_id,'A3',secret);
...
begin
  ...
  elsif lower(p_bot_key)='efe' then
    if bot_attempt_no=1 then
      efe_green_cap:=0;
    elsif bot_attempt_no=2 then
      efe_green_cap:=1;
  ...
  where w.answer_version='A3'
    ...
    and (efe_green_cap is null or coalesce((public.live_feedback(w.word,p_answer)->>'green')::integer,0) <= efe_green_cap)
    and (
  ...
  order by (
    coalesce((public.live_feedback(w.word,p_answer)->>'green')::integer,0)
      * case lower(p_bot_key) when 'atlas' then 6 when 'defne' then 3.4 when 'efe' then 1.8 else 4 end
    + coalesce((public.live_feedback(w.word,p_answer)->>'yellow')::integer,0)
      * case lower(p_bot_key) when 'atlas' then 2.5 when 'defne' then 1.5 when 'efe' then 0.8 else 1.5 end
    + random()*case lower(p_bot_key) when 'atlas' then 0.7 when 'defne' then 3.2 when 'efe' then 6.8 else 4.0 end
  ) desc
  limit 1;
  ...
  if chosen is null then
    ...
    order by (
      coalesce((public.live_feedback(w.word,p_answer)->>'green')::integer,0) * 4
      + coalesce((public.live_feedback(w.word,p_answer)->>'yellow')::integer,0) * 1.5
      + random()*1.4
    ) desc
    limit 1;
  end if;
