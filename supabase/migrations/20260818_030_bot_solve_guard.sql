-- Kelimelik — bot solve guard and Defne tuning
-- Prevents a bot from jumping from low visible information straight to the answer.
-- Efe / Defne / Atlas keep distinct curves; visible green and green+yellow never regress.
...
begin
  ...
  /*
   * Target turn is only an earliest intention now; advance_bot_match also
   * requires enough visible information before the answer may be submitted.
   * Defne is slightly slower than before, especially in 8-attempt Kelimelik.
   */
...
declare
  ...
  max_step_present integer := 0;
  max_step_green integer := 0;
begin
  ...
  /* Defne/Efe gain at most one visible present letter per normal step.
     Atlas may gain two because it is the intentionally strong bot. */
  max_step_present:=least(
    p_length-1,
    max_present + case when bot_key='atlas' then 2 else 1 end
  );
  max_step_green:=least(
    p_length-1,
    max_green + case when bot_key='atlas' then 2 else 1 end
  );

  /* First choice: fully consistent remaining candidate with a smooth step. */
  with candidate_words as (
    ...
    cross join lateral (select public.live_feedback(c.word,p_answer) as fb) f
  )
  select s.word into chosen
  from scored s
  where s.green_count>=max_green
    and s.present_count>=max_present
    and s.green_count<=max_step_green
    and s.present_count<=max_step_present
    and not exists(
      select 1
  ...
  /* Second choice: probe outside the remaining-secret set but keep learned
     information, known Classic green positions and the smooth step cap. */
  ...
  /* Last non-answer fallback: if a +1 step is impossible, take the smallest
     possible monotonic overshoot. The secret itself is still excluded. */
  if chosen is null then
    with candidate_words as (
      select w.word,
             case when exists(
    ...
    order by s.present_count asc,s.green_count asc,s.pool_rank,random()
    limit 1;
  end if;
...
begin
  ...
  then
    ...
    /* A target turn is not enough by itself. The bot must first visibly
       approach the answer. This removes 2/6 -> answer jumps. */
    solve_floor:=case lower(m.bot_key)
      when 'efe' then greatest(2,m.word_length-3)
      when 'atlas' then greatest(2,m.word_length-2)
      else greatest(3,m.word_length-1)
    end;
    ...
    /* Extremely rare exhausted-decoy case: do not reveal the answer before
       the solve guard is satisfied. Let the bot think and retry later. */
    if guess is null then
      update public.bot_matches
      set bot_next_guess_at=clock_timestamp()+make_interval(secs=>public.bot_delay_seconds(bot_key)),
